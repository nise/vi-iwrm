...
//		$('body').prepend(this.buildDropDown(this.getAuthors(), 'choose lecturer', null));		
		...
		$.ajax({type: "POST", dataType: "json", url: './ocr.json', 
			...
			$.each(string.split(" "), function(i, str){ 
					...
					$.each(_this.json_data._stream, function(i, stream){ 
							...
							//var slides = '';
							$.each(res.ocr, function(index, val){ 
								if(val.id == stream.id){ 
	...
	getTestPage : function(){
		...
		var loadthevideo = function(stream, the_page, name){
			var template = $("#memex_template").html();
...
var Video = $.inherit(
{
  ...
  __constructor: function(options, main) {
	...
	name: 'video player',
	// defaults
	options: {main: null, selector: '#video1', width: 500, height: 375, wrapControls: '', childtheme: '', thumbnail:'img/placeholder.jpg'},
	video: null,
	main: null,
	url: '',
	video_controls: null,
